...
FIXED: Corrected paths and check logic for actual infrastructure
"""
...
UTILITIES_HOST = '192.168.40.10'  # This host for DNS resolution
OPNSENSE_URL = 'https://192.168.91.30'
AUTHENTIK_URL = 'http://192.168.40.21:9000'
DOMAIN = 'hrmsmrflrii.xyz'

# Service to VM mapping - containers run on these VMs
SERVICE_VM_MAP = {
    # docker-vm-utilities01 services
    'uptime': 'docker-vm-utilities',
    'prometheus': 'docker-vm-utilities',
    'grafana': 'docker-vm-utilities',
    'glance': 'docker-vm-utilities',
    'n8n': 'docker-vm-utilities',
    'paperless': 'docker-vm-utilities',
    'speedtest': 'docker-vm-utilities',
    'jaeger': 'docker-vm-utilities',
    'demo': 'docker-vm-utilities',
    # docker-vm-media01 services
    'jellyfin': 'docker-vm-media',
    'radarr': 'docker-vm-media',
    'sonarr': 'docker-vm-media',
    'lidarr': 'docker-vm-media',
    'prowlarr': 'docker-vm-media',
    'bazarr': 'docker-vm-media',
    'overseerr': 'docker-vm-media',
    'jellyseerr': 'docker-vm-media',
    'tdarr': 'docker-vm-media',
    'autobrr': 'docker-vm-media',
    'deluge': 'docker-vm-media',
    'sabnzbd': 'docker-vm-media',
    # Dedicated VMs
    'traefik': 'traefik-vm',
    'authentik': 'authentik-vm',
    'auth': 'authentik-vm',
    'photos': 'immich-vm',
    'immich': 'immich-vm',
    'gitlab': 'gitlab-vm',
...
# ============================================================================
# Onboarding Checker Functions - FIXED PATHS
# ============================================================================
...
def check_terraform(service_name: str) -> bool:
    """Check if service's VM has Terraform configuration."""
    # Map service to its VM name prefix
    vm_prefix = SERVICE_VM_MAP.get(service_name, service_name)

    # Check main.tf on the utilities host (has access to the repo)
    # The main.tf contains VM group names like "docker-vm-utilities", "docker-vm-media", etc.
    output = ssh_command(
        UTILITIES_HOST,
        f"grep -qi '{vm_prefix}' /home/hermes-admin/tf-proxmox/main.tf 2>/dev/null && echo 'found' || "
        f"grep -qi '{service_name}' /home/hermes-admin/tf-proxmox/main.tf 2>/dev/null && echo 'found'"
    )
    if output and 'found' in output:
        return True

    # Fallback: Check if it's a container service (most are)
    # Container services don't have individual Terraform configs
    if service_name in SERVICE_VM_MAP:
        # Check if the parent VM exists in Terraform
        output = ssh_command(
            UTILITIES_HOST,
            f"grep -qi '{vm_prefix}' /home/hermes-admin/tf-proxmox/main.tf 2>/dev/null && echo 'found'"
        )
        return output and 'found' in output

    return False


def check_ansible(service_name: str) -> bool:
    """Check if service has Ansible playbook or docker-compose on host."""
    # Check for playbook in ~/ansible/ on the controller
    # Search recursively through all subdirectories
    output = ssh_command(
        ANSIBLE_HOST,
        f"ls -d ~/ansible/{service_name}/ 2>/dev/null || "
        f"ls ~/ansible/{service_name}*.yml 2>/dev/null || "
        f"ls ~/ansible/*/{service_name}*.yml 2>/dev/null || "
        f"grep -rqi '{service_name}' ~/ansible/ 2>/dev/null && echo 'found'"
    )
    if output:
        return True
...
def check_dns(service_name: str) -> bool:
    """Check if service has DNS record - use SSH to resolve from network."""
    # Use nslookup from a host that has access to internal DNS
    output = ssh_command(
        UTILITIES_HOST,
        f"nslookup {service_name}.{DOMAIN} 2>/dev/null | grep -q 'Address:' && echo 'resolved'"
    )
    if output and 'resolved' in output:
        return True

    # Alternative: dig command
    output = ssh_command(
        UTILITIES_HOST,
        f"dig +short {service_name}.{DOMAIN} 2>/dev/null"
    )
    if output and output.strip():
        return True

    # Fallback to OPNsense API if credentials available
    if OPNSENSE_API_KEY and OPNSENSE_API_SECRET:
        try:
...
def check_documentation(service_name: str) -> bool:
    """Check if service is documented in SERVICES.md."""
    # Check on utilities host where repo might be cloned
    output = ssh_command(
        UTILITIES_HOST,
        f"grep -qi '{service_name}' /home/hermes-admin/tf-proxmox/docs/SERVICES.md 2>/dev/null && echo 'found'"
    )
    if output and 'found' in output:
        return True

    # Check on ansible controller
    output = ssh_command(
        ANSIBLE_HOST,
        f"grep -qi '{service_name}' ~/ansible/README.md 2>/dev/null && echo 'found'"
    )
    if output and 'found' in output:
        return True
...
def generate_report(services=None) -> str:
    ...
    for service in services:
        ...
        row = (
            f"{service[:15]:<15} | "
            f"{'✓' if checks['terraform'] else '✗':^3} | "
            f"{'✓' if checks['ansible'] else '✗':^3} | "
            f"{'✓' if checks['dns'] else '✗':^3} | "
            f"{'✓' if checks['traefik'] else '✗':^4} | "
            f"{'✓' if checks['ssl'] else '✗':^3} | "
            f"{'✓' if checks['authentik'] else '-':^4} | "
            f"{'✓' if checks['documentation'] else '✗':^4}"
        )
        rows.append(row)
